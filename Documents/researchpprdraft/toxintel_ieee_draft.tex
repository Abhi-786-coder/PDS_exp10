...
Candidate reliability is assessed using Tanimoto-based OOD distance checks and
...
optimization, Tox21, drug discovery, cheminformatics.
\end{IEEEkeywords}
...
a two-layer shared backbone with Batch Normalization, GELU activations, and
dropout, branching into 12 endpoint-specific output heads.
...
(ii) applies per-endpoint alpha weighting, and (iii) dynamically suppresses
gradient contributions from easily classified majority samples via
the focusing exponent $\gamma = 2.0$.
Hyperparameters were optimized via Optuna search (up to 200 trials) with a
...
bioisostere retrieval filtered for synthesizability (SAScore\,$<$\,4.5 and
SCScore\,$<$\,4.0) and ADME preservation ($|\Delta\text{LogP}| < 1.5$, $|\Delta\text{MW}| < 80$\,Da);
and Pareto multi-endpoint dominance ranking to guarantee global multi-organ
safety improvements.
Final candidate reliability is assessed using Tanimoto-based OOD distance
checks against the training set.
\end{enumerate}
  ...
  The DeepTox framework~\cite{mayr2016deeptox}, a winner of the Tox21 Challenge,
  demonstrated that deep networks trained on chemical descriptors consistently
  outperform shallow ML baselines across toxicity endpoints, establishing deep
  learning as the de facto standard for this class of problems.
...
landscapes \cite{ramsundar2015,zhang2025}.
...
GELU activations and the Per-Endpoint Masked Focal Loss.
...
computations.
To determine the optimal molecular representation, we conducted an empirical
ablation study across candidate Morgan fingerprint configurations.
The best-performing representation was 4096-bit ECFP4, which became the
primary input to the ToxNet pipeline.
Descriptors were computed for exploratory analysis, but the final prediction
pipeline uses 4096-bit fingerprint embeddings exclusively.
All descriptor-derived scalers were fit only on the training partition to
...
\caption{Featurization ablation study. The 4096-bit ECFP4 fingerprint representation consistently outperforms all other featurizations in macro AUPRC.}
...
toxic minority class and predicts inefficacy of ADASYN~\cite{he2008adasyn} oversampling in embedding space.

\subsection{ToxNet Architecture and Focal Loss}
ToxNet is a PyTorch multi-task neural network with a shared backbone of two
hidden layers with Batch Normalization, GELU activation, and dropout.
...
All models are optimized using the AdamW optimizer~\cite{loshchilov2019adamw} with
gradient norm clipping (max-norm 1.0) for stable Focal Loss convergence.
Hyperparameters were optimized via Optuna~\cite{akiba2019optuna} search (up to 200 trials)
with a cosine annealing schedule and early stopping at 10 stagnant trials (patience=15).

  \subsection{Prediction-to-Prescription Pipeline}

  \subsubsection{Step 1: Attribution}
  SHAP GradientExplainer~\cite{lundberg2017} (Expected Gradients) is applied to ToxNet predictions to obtain bit-level
  ...
  SHAP-identified fragments are cross-referenced against two structural alert
  catalogs built into RDKit's \texttt{FilterCatalog}: PAINS (Pan-Assay Interference
  Compounds, sub-catalogs A, B, and C) and the Brenk filter.
  Fragments confirmed by at least one catalog receive a \textit{high-confidence}
...
database cache (with live API fallback) to retrieve known bioisosteric replacements.
Candidates are filtered by a 2-metric synthesizability consensus:
SAScore~\cite{ertl2009sascore}\,$<$\,4.5 and SCScore~\cite{coley2018scscore}\,$<$\,4.0.
ADME preservation is verified using Lipinski drug-likeness criteria~\cite{lipinski2001}:
$|\Delta\text{LogP}| < 1.5$ and $|\Delta\text{MW}| < 80$\,Da.
...
Candidate reliability is assessed using a training-set Tanimoto similarity
OOD check rather than a conformal prediction layer.
  ...
  Macro AUPRC is the primary metric~\cite{davis2006} due to the severe class imbalance
...
ToxNet (ours)         & ECFP4 (4096) & \textbf{0.842} & \textbf{0.364} \\
...
\caption{ToxNet training and validation loss curves. Cosine annealing ensures stable convergence. Early stopping at patience=15 prevents overfitting.}
\label{fig_training}
\end{figure}
...
For these endpoints, embedding-space ADASYN yielded negligible or negative
changes in AUPRC ($\Delta$AUPRC $\leq 0.005$), confirming the predicted
failure mode.
For the remaining six endpoints with $\rho < 1.5$, adaptive oversampling produced
...
cohesion ratio and the ADASYN-induced AUPRC change, providing direct
empirical evidence for the geometric imbalance hypothesis.

  \begin{figure}[!t]
  \centering
  \includegraphics[width=3.0in]{05_geometric_correlation.png}
  \caption{Correlation between per-endpoint Intraclass Tanimoto Cohesion ratio and the change in AUPRC from embedding-space ADASYN. Endpoints with $\rho > 1.5$ show negligible or negative benefit from oversampling, confirming the geometric imbalance hypothesis.}
  \label{fig_geom_corr}
  \end{figure}
...
Tanimoto similarity-based OOD checks correctly flagged candidates with training-set distance $>0.7$, supporting the reliability assessment of proposed replacements.
  ...
  Second, SAScore~\cite{ertl2009sascore} is a heuristic synthesizability estimate;
  genuine chemical feasibility requires retrosynthetic pathway analysis.
  ...
  Finally, Mondrian conformal prediction~\cite{vovk2005} was evaluated as an
  alternative uncertainty quantification layer, but Tanimoto-based OOD distance
  checks were adopted in its place due to lower runtime overhead and direct
  interpretability in chemical space.
  ...
  substitution and Tanimoto-validated OOD reliability assessment, ToxIntel delivers
  ...
  \bibitem{mayr2016deeptox}
  A. Mayr, G. Klambauer, T. Unterthiner, and S. Hochreiter, ``DeepTox:
  Toxicity prediction using deep learning,'' \textit{Front. Environ. Sci.},
  vol.~3, p.~80, 2016.

  \bibitem{ramsundar2015}
  B. Ramsundar, R. B. Bharath, H. Kell, and V. S. Pande, ``Massively multitask
  networks for drug discovery,'' \textit{arXiv preprint arXiv:1502.02072}, 2015.

  \bibitem{he2008adasyn}
  H. He, Y. Bai, E. A. Garcia, and S. Li, ``ADASYN: Adaptive synthetic
  sampling approach for imbalanced learning,'' in \textit{Proc. IEEE Int.
  Joint Conf. Neural Networks (IJCNN)}, 2008, pp.~1322--1328.
  ...
  ACM SIGKDD Int. Conf. Knowledge Discovery and Data Mining}, 2019,
  pp.~2623--2631.
  ...
  \bibitem{coley2018scscore}
  C. W. Coley, L. Rogers, W. H. Green, and K. F. Jensen, ``SCScore: Synthetic
  complexity learned from a reaction corpus,'' \textit{J. Chem. Inf. Model.},
  vol.~58, no.~2, pp.~252--261, 2018.

  \bibitem{ertl2009sascore}
  P. Ertl and A. Schuffenhauer, ``Estimation of synthetic accessibility score
  of drug-like molecules based on molecular complexity and fragment
  contributions,'' \textit{J. Cheminformatics}, vol.~1, p.~8, 2009.
  ...
  \textit{Adv. Drug Delivery Rev.}, vol.~46, nos.~1--3, pp.~3--26, 2001.

  \bibitem{davis2006}
  J. Davis and M. Goadrich, ``The relationship between Precision-Recall and
  ROC curves,'' in \textit{Proc. Int. Conf. Machine Learning (ICML)}, 2006,
  pp.~233--240.
